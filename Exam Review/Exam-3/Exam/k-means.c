...
void* thread_main(void* thread_arg) {
    ...
    // adds labels
    for(int l = 0; l < rounds; l++)
    // for loop used to go through every centroid
    {
        compute_labels(centroids, k, a, m, labels);
        compute_centroids(centroids, k, a, m, labels);
    }

    // uses the squared_intra function to add to the struct 
...
int main(int argc, char *argv[])
{
    ...
    // for loop used to create the threads
    for(int i = 0; i < n_threads; i++) {
    ...
    // join threads to make sure no zombies
    for(int i = 0; i < n_threads; i++) {
        rv = pthread_join(thread_arr[i], NULL);
    ...
    // uses the array in struct
    double best_d = squared_intra_cluster_dist[0];
    // set best centroid which is then used in the for loop below
    point_t *best_centroids = (point_t *)&centroids[0];

    for(int i = 0; i < n_threads; i++) {
        if(squared_intra_cluster_dist[i] < best_d) {
            best_d = squared_intra_cluster_dist[i];
            best_centroids = (point_t *)&centroids[i];
        }
    }

    // prints the best centroids
    print_clusters(best_centroids, k, a, m, labels);
    printf("Squared intra-cluster distance: %lf\n", best_d);
