...
void * thread_main(void * arg_in)
{
    ...
    // create a buf with MSG_BUF_SIZE to store numbers as strings of decimal digits
    char buf[MSG_BUF_SIZE];

    // declare guess and result int vars
    int guess, result;

    // convert the max integer to decimal string
    snprintf(buf, MSG_BUF_SIZE, "%d\n", gmn_get_max());

    // send the max numbers to the client
    if (send_str(sockfd, buf)) {
        // error handling
        fprintf(stderr, "send_line() returned -1.\n");
        close(sockfd);
        free(arg);
        return NULL;
    }
    ...
    while (!done) {
        ...
        if (recv_lines(sockfd, buf, MSG_BUF_SIZE)) {
            fprintf(stderr, "recv_int() returned -1.\n");
            close(sockfd);
            free(arg);
            return NULL;
        }

        // use atoi to convert back into number from string
        guess = atoi(buf);
        ...
        // the guess is correct - then result is 0
        if (result == 0) {
            // send result
            snprintf(buf, MSG_BUF_SIZE, "%s", gmn_get_message(&gmn));
            if (send_str(sockfd, buf)) {
                fprintf(stderr, "send_str() returned -1.\n");
                close(sockfd);
                free(arg);
                return NULL;
            }
