...
void * thread_c(void * arg_in)
{
    ...
    // get max - assign it to arg->max - send it to thread_p
    arg->max = gmn_get_max();
    // change status to S_MAX to signal thread_p that max was sent
    arg->status = S_MAX;
    // send signal 
    ...
    do{
        // lock
        pthread_mutex_lock(&arg->mutex);
        // if a guess hasn't been recieved - wait 
        while(arg->status != S_GUESS){
            pthread_cond_wait(&arg->cond_guess, &arg->mutex);
        }

        // after leaving while loop - check the guess and assign the result to arg->result
        arg->result = gmn_check(&gmn, arg->guess);

        // if the result is correct - copy the string message into arg and send it to thread_p
        if(arg->result == 0){
            strcpy(arg->message, gmn_get_message(&gmn));
            done = 1;
        }

        // change the status to S_RESULT in order to signal thread_p that the result was added to arg
        arg->status = S_RESULT;
        // signal to thread_p
        pthread_cond_signal(&arg->cond_result);
        //unlock
...
void * thread_p(void *arg_in)
{
    ...
    do { 
        ...
        //lock
        pthread_mutex_lock(&arg->mutex);
        // assign the guess that was calculated to arg->guess - saved into arg
        arg->guess = guess;
        // change the status to S_GUESS to signal to thread_c that there is a guess
        arg->status = S_GUESS;
        // signal that there is a guess
        pthread_cond_signal(&arg->cond_guess);

        // wait until there is a result from the guess
        while(arg->status != S_RESULT){
            pthread_cond_wait(&arg->cond_result, &arg->mutex);
        }

        // the result from thread_c is then assigned to var result
        result = arg->result;
