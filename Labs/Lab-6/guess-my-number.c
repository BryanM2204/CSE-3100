...
void    child_main(int fdp[], int fdc[], int seed)
{
    ...
    // closed file descriptors that are not being used
    close(fdp[1]);
    close(fdc[0]);

    // call on gmn_get_max() and get max
    int max = gmn_get_max();
    // child writes to the parent the max value
    write(fdc[1], &max, sizeof(int));

    int result = -1, guess;

    // while loop that gets guess from parent 
    // check guess with gmn_check
    ...
    // final_message array created with MSG_BUF_SIZE size
    char final_message[MSG_BUF_SIZE];
    // strcpy used to copy the returned string from gmn_get_message to final_message
    strcpy(final_message, gmn_get_message(&gmn));
    // child writes to parent the final_message
    write(fdc[1], final_message, MSG_BUF_SIZE * sizeof(char));
...
int main(int argc, char *argv[])
{
    ...
    // close pipe file descriptor that is not being used
    close(fdp[0]);

    // parent reads from child the max value
    ...
    do { 
        ...
        // parent writes to child its guess
        write(fdp[1], &guess, sizeof(guess));
        // parent reads from what child sent through pipe - which is the result of the guess
    ...
    // create char array with MSG_BUF_SIZE size
    char final_message[MSG_BUF_SIZE];
    // parent reads from the child the final message and stores it in the final message array
    read(fdc[0], final_message, MSG_BUF_SIZE * sizeof(char));
    // prints out the final message
    printf("%s", final_message);

    // close any file descriptors remaining
    close(fdp[1]);
    close(fdc[0]);
    close(fdc[1]);
