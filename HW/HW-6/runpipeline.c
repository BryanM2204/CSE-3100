...
void start_program(Program *programs, int num_programs, int cur) 
{
    ...
    if(child==-1){
        ...
        // the currrent program's fd for reading is dup to FD_STDIN - recieve from other programs info
        // writing duped to FD_STDOUT - send to the next program
        dup2(programs[cur].fd_in, FD_STDIN);
        dup2(programs[cur].fd_out, FD_STDOUT);

        // after using dup2 - close uneccessary file descriptors
        close(programs[cur].fd_in);
        close(programs[cur].fd_out);

        // create new process of next program
        execvp(programs[cur].argv[0], programs[cur].argv);

        // if execvp fails - output error
        die("execvp() failed.");
        ...
        // close the uncessary file descriptors
        close(programs[cur].fd_in);
        close(programs[cur].fd_out);
    }
}
...
int wait_on_program(Program *prog)
{
    ...
    // wait on the child process to finish succesfully 
...
void prepare_pipes(Program *programs, int num_programs)
{
    ...
    // create a for loop to create pipes connecting every program 
     for(int i=0; i<num_programs-1; i++){
        int pd[2];
        // check that the pipe was created correctly - send error if it fails
        if(pipe(pd) == -1){
            die("pipe() failed");
        }
        // assign the writing side of the pipe to the fd_out of the program (data that program needs to send out)
        programs[i].fd_out = pd[PIPEFD_WRITE];
        // assign the reading side of the pipe to the fd_in of the program (data that needs to be recieved)
