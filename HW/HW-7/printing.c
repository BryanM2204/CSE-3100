...
void * printer_main(void * arg)
{
    ...
    // assigned printer to the arguments passed into the function
    printer_t *printer = arg;
    // assigned jq to job_queue_t
    ...
    while(! done){
        ...
        // asisgn d to number of jobs to be printed
        d = q_num_jobs(jq);
        
        // if d is less than or equal to 0
        if(d <= 0){
            // then job is done and assign 1 to done
            done = 1;
            // unlock mutex
        ...
        // after job is done - unlock mutex
        pthread_mutex_unlock(&jq->mutex);
        
        // print the job
        print_job(job);

        // incrememnt number of jobs completed
        printer->njobs++;
    }
    return arg;
}
...
int main(int argc, char *argv[])
{
    ...
    // created for loop to initiliaze arguments of each thread, or printer, and create thread using pthread_create
    ...
    // free up space by destroying mutex
